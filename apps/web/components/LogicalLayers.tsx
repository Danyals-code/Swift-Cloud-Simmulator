...
import { rebaseSourceLayers, sourceLayerHiddenOwner, sourceLayerIsVisual, sourceLayerLabel, sourceLayerNotShown, sourceLayerRows, sourceLayerType, sourceLayerVisibleId, sourceLayerPrimaryViewId, type SourceLayerNavigation } from '../lib/sourceLayers'
...
interface Props {
  ...
  onHover?: (node: AuthoringNode | null) => void
  stale: boolean
...
export function LogicalLayers({ snapshot, files, selected, selectedAncestors = [], selection, hovered, hoveredAncestors = [], onHover, stale, onSelect, onEdit, hidden = [], onShow, editable = false }: Props) {
  ...
  const { rows, entry } = sourceLayerRows(snapshot, current, selected, query)
  ...
  useEffect(() => () => onHover?.(null), [snapshot, rowIds, entry?.id, query, onHover])
      ...
      {rows.map(({ node, depth, expanded: modelExpanded, parentId, children }, index) => {
        ...
        return <Fragment key={node.id}>
          <div className={styles.row} style={{ paddingLeft: 8 + displayDepth * 14 }} data-source-id={node.id} data-source-name={node.name} data-source-owner={node.owner} data-source-kind={node.kind} role="treeitem" aria-label={`${label}${label !== type ? `, ${type}` : ''}`} aria-level={displayDepth + 1} aria-selected={selectedRow === node.id} data-hovered={hoveredRow === node.id || undefined} aria-expanded={expandable ? expanded : undefined} tabIndex={tabStop === node.id ? 0 : -1} data-inactive={notShown || undefined} data-dragging={drag?.id === node.id || undefined} data-drop={drag?.over === node.id ? drag.position : undefined} draggable={canDrag}
            onMouseEnter={() => { if (!stale) onHover?.(node) }} onMouseLeave={() => onHover?.(null)}
            ...
            <span className={styles.kind}>{notShown ? 'Not shown' : node.kind === 'component' ? 'Component' : label !== type ? type : ''}</span>
