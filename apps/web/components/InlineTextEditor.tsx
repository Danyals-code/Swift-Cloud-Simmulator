...
export function InlineTextEditor({ value, x, y, width, onSave, onClose }: { value: string; x: number; y: number; width: number; onSave: (value: string) => Promise<string | null>; onClose: () => void }) {
  ...
  const panelWidth = Math.min(Math.max(260, width), window.innerWidth - 16)
  return createPortal(<form className={styles.inlineText} style={{ left: Math.max(8, Math.min(x, window.innerWidth - panelWidth - 8)), top: Math.max(8, Math.min(y, window.innerHeight - 180)), width: panelWidth }} aria-label="Edit canvas text" onSubmit={event => { event.preventDefault(); void save() }} onKeyDown={event => { event.stopPropagation(); if (event.key === 'Escape') { event.preventDefault(); if (!busy) onClose() } }}>
    ...
    <div><small>Enter to save · Esc to cancel</small><button type="button" disabled={busy} onClick={onClose}>Cancel</button><button type="submit" disabled={busy}>Save</button></div>{error && <p role="alert">{error}</p>}
  </form>, document.body)
}
