...
export function defaultRecord(info: Pick<CollectionSettings, 'fields'>, records: readonly DesignRecord[]): DesignRecord {
  ...
  let index = 1
  while (records.some(r => r.id === (id.type === 'String' ? `item-${index}` : index))) index++
...
export function RecordEditor({ info, value, onChange }: { info: Pick<CollectionSettings, 'fields'>; value: readonly DesignRecord[]; onChange: (value: readonly DesignRecord[]) => void }) {
  ...
  return <div className={styles.controls} data-testid="record-editor">
    <div className={styles.control}><label>Record<select aria-label="Selected record" value={record ? index : ''} onChange={e => setSelected(Number(e.target.value))}>{!record && <option value="">Empty collection</option>}{value.map((r, i) => <option key={i} value={i}>{i + 1}. {String(r.id)}</option>)}</select></label></div>
    {record && info.fields.map(field => <div key={field.name} className={styles.control}><label>{field.name}{field.type === 'Bool' ? <select aria-label={`Record ${field.name}`} disabled={record[field.name] === null} value={String(record[field.name] ?? false)} onChange={e => update(field.name, e.target.value === 'true')}><option value="false">False</option><option value="true">True</option></select> : <input aria-label={`Record ${field.name}`} type="text" inputMode={['Int', 'Double'].includes(field.type) ? 'decimal' : 'text'} disabled={record[field.name] === null} value={String(record[field.name] ?? '')} onChange={e => update(field.name, field.type === 'String' ? e.target.value : e.target.value === '' ? null : Number(e.target.value))} />}</label>
