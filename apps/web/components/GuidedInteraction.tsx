...
export function GuidedInteraction({ node, snapshot, onSelect, onCommand, busy }: { node: AuthoringNode; snapshot?: AuthoringSnapshot; onSelect?: (node: AuthoringNode) => void; onCommand: (op: AuthoringOperation) => Promise<void>; busy: boolean }) {
  ...
  const [kind, setKind] = useState<'navigate' | 'sheet' | 'toggle' | 'set'>('navigate')
  ...
  const currentActionLabel = !hasAction ? 'No action yet' : presentation ? `Open ${presentation.navigation!.display} as a sheet` : toggleAction ? `Switch ${readable(toggleAction[1]!)} on / off` : setAction ? `Change ${readable(setAction[1]!)} to ${setAction[2]}` : 'Existing custom action (see Advanced actions)'
  ...
  const apply = () => {
    const newScreen = destination === '__new' && (kind === 'navigate' || kind === 'sheet') ? { name: uniqueName(screenName, snapshot, 'Screen'), title: screenName.trim() } : undefined
    ...
    const action: BehaviorAction = kind === 'navigate' || kind === 'sheet' ? { type: kind, destination: newScreen?.name ?? destination } : kind === 'toggle' ? { type: 'toggle', state: createValue?.name ?? state } : { type: 'set', state: createValue?.name ?? state, value: scalar(value || (type === 'Bool' ? 'true' : value)) }
  ...
  return <div className={styles.guidedInteraction}>
    {info.canConfigureAction && <>
      <p><strong>Current:</strong> {currentActionLabel}</p>
      <label>{hasAction ? 'Change interaction' : 'On tap'}<select aria-label="On tap" value={kind} onChange={e => { setKind(e.target.value as typeof kind); if (e.target.value === 'toggle') { setValueType('Bool'); setInitial('false'); if (current?.type !== 'Bool') setState('__new') } }}><option value="navigate">Go to screen</option><option value="sheet">Open sheet</option><option value="toggle">Change a value · switch on/off</option><option value="set">Change a value · set value</option></select></label>
      {kind === 'navigate' || kind === 'sheet' ? <>
        ...
        <p>{kind === 'navigate' ? 'Opens this screen with a Back button. Navigation is set up automatically.' : 'Opens this screen above the current screen. Drag down to close it.'}</p>
      </> : <>
      ...
      <button type="button" disabled={busy || hasAction && !replace || invalidNumber || (kind === 'navigate' || kind === 'sheet') && destination === '__new' && !screenName.trim() || (kind === 'toggle' || kind === 'set') && state === '__new' && !valueName.trim()} onClick={apply}>Save interaction</button>
    </>}
