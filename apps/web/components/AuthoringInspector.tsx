  ...
  return <div ref={root} className={styles.inspector} data-testid="authoring-inspector" aria-busy={stale}
    ...
    <fieldset disabled={stale} className={styles.inspectorFields}>
      ...
      {hasData && <section className={styles.settingsSection} data-testid="settings-data"><h3>Data</h3>
        ...
        {features && (selected.name !== 'List' || !nestedCollections.length || selected.kind === 'collection') && <AuthoringFeatures key={`data:${featureKey}`} node={selected} {...features} section="data" />}
        {nestedCollections.map(collection => <div key={`collection:${sourceKey}:${collection.id}:${collection.fingerprint}:${collection.collection?.signature}`} data-settings-owner={`${collection.owner}:${collection.name}:${collection.source.file}:${collection.source.start}`}><AuthoringFeatures node={collection} {...forNode(collection)} section="data" /></div>)}
      </section>}
