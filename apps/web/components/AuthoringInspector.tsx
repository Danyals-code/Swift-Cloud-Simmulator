...
export function AuthoringInspector({ node, stale, onReveal, onChange, features }: { features?: Omit<FeatureProps, 'node'>; node?: AuthoringNode; stale?: boolean; onReveal?: (span: SourceSpan) => void; onChange?: PropertyChange }) {
  ...
  const focus = useRef<{ source: string; control: string; label: string; caret: number | null } | null>(null)
  ...
  useLayoutEffect(() => {
    ...
    const input = Array.from(root.current?.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>('input, select, textarea') ?? []).find(element => previous.control ? element.dataset.controlId === previous.control : element.getAttribute('aria-label')?.toLowerCase() === previous.label)
    if (!input) return
  ...
  let parent = features?.snapshot?.nodes.find(candidate => candidate.id === selected.parentId)
  while (parent) { ancestors.unshift(parent); parent = features?.snapshot?.nodes.find(candidate => candidate.id === parent!.parentId) }
  const template = selected.kind === 'template' || ancestors.some(ancestor => ancestor.kind === 'template')
  const hasData = selected.name === 'List' || selected.kind === 'collection' || !!selected.fields?.length
  const hasBehavior = !!selected.modifiers?.some(modifier => modifier.category === 'behavior') || !!selected.behavior && (selected.behavior.canConfigureAction || !!selected.behavior.binding || selected.behavior.states.length > 0)
  ...
  const rememberFocus = (input: EventTarget) => {
    if (input instanceof HTMLInputElement || input instanceof HTMLSelectElement || input instanceof HTMLTextAreaElement) focus.current = { source: sourceKey, control: input.dataset.controlId ?? '', label: input.getAttribute('aria-label')?.toLowerCase() ?? '', caret: input instanceof HTMLInputElement || input instanceof HTMLTextAreaElement ? input.selectionStart : null }
  }
  return <div ref={root} className={styles.inspector} data-testid="authoring-inspector" aria-busy={stale}
    ...
    <fieldset disabled={stale} className={styles.inspectorFields}>
      ...
      {!!ancestors.length && <nav className={styles.breadcrumbs} aria-label="Selection path">{ancestors.map(ancestor => <button type="button" key={ancestor.id} onClick={() => features?.onSelect?.(ancestor)}>{sourceLayerLabel(ancestor)}</button>)}</nav>}
      ...
      {!['definition', 'template', 'branch'].includes(selected.kind) && <section className={styles.settingsSection}><ModifierStack key={sourceKey} node={selected} onChange={onChange} onCommand={features?.onCommand} onReveal={onReveal} /></section>}
      {hasData && <section className={styles.settingsSection} data-testid="settings-data"><h3>Data</h3>{features && <AuthoringFeatures key={`data:${featureKey}`} node={selected} {...features} section="data" />}</section>}
      {hasBehavior && <section className={styles.settingsSection} data-testid="settings-behavior"><h3>Behavior</h3><ModifierStack key={`behavior:${sourceKey}`} node={selected} onChange={onChange} onCommand={features?.onCommand} onReveal={onReveal} behavior />{features && <AuthoringFeatures key={`behavior:${featureKey}`} node={selected} {...features} section="behavior" />}</section>}
