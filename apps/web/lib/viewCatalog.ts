 ...
 * What Add can add.
 *
 * Every snippet here is Swift that compiles on its own, with no state behind it.
 * That is the whole design constraint: a control needs a binding, and a view added
 * to a screen cannot invent an `@State` property to bind to - so the ones that need
 * a binding get `.constant(…)`, which reads a value and swallows what is written.
 * The view draws, the file still compiles, and wiring it to real state is an edit
 * the person makes in the editor, where it belongs.
 *
 * The alternative - adding a property to the enclosing struct at the same time -
 * is two edits to two places from one click, and getting the second one wrong is
 * how a visual editor starts producing files people have to repair by hand.
 */
...
export const VIEW_CATALOG: readonly ViewSnippet[] = [
  // --------------------------------------------------------------- Layout
  { id: 'vstack', name: 'VStack', group: 'Layout', hint: 'Stack views top to bottom', keywords: 'column vertical', snippet: 'VStack {\n    Text("Item")\n}' },
  { id: 'hstack', name: 'HStack', group: 'Layout', hint: 'Stack views side by side', keywords: 'row horizontal', snippet: 'HStack {\n    Text("Item")\n}' },
  { id: 'zstack', name: 'ZStack', group: 'Layout', hint: 'Stack views front to back', keywords: 'overlay depth layer', snippet: 'ZStack {\n    Text("Item")\n}' },
  ...
  { id: 'scrollview', name: 'ScrollView', group: 'Layout', hint: 'Scroll its contents', keywords: 'scrolling', snippet: 'ScrollView {\n    VStack {\n        Text("Item")\n    }\n}' },
  ...
  { id: 'grid', name: 'LazyVGrid', group: 'Layout', hint: 'A grid of columns', keywords: 'columns tiles', snippet: 'LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())]) {\n    Text("Cell")\n}' },
  { id: 'foreach', name: 'ForEach', group: 'Layout', hint: 'One view per element', keywords: 'loop repeat', snippet: 'ForEach(0..<3, id: \\.self) { index in\n    Text("Row \\(index)")\n}' },
  ...
  { id: 'labeledcontent', name: 'LabeledContent', group: 'Content', hint: 'A title with a value', keywords: 'row detail value', snippet: 'LabeledContent("Title", value: "Value")' },
  ...
  { id: 'progress', name: 'ProgressView', group: 'Content', hint: 'A bar or a spinner', keywords: 'loading spinner bar', snippet: 'ProgressView(value: 0.5)' },
  { id: 'groupbox', name: 'GroupBox', group: 'Content', hint: 'A titled card', keywords: 'card panel', snippet: 'GroupBox("Title") {\n    Text("Content")\n}' },
  ...
  { id: 'textfield', name: 'TextField', group: 'Controls', hint: 'A single line of input', keywords: 'input entry', snippet: 'TextField("Placeholder", text: .constant(""))' },
  ...
  { id: 'datepicker', name: 'DatePicker', group: 'Controls', hint: 'Pick a date', keywords: 'calendar time', snippet: 'DatePicker("Date", selection: .constant(Date()))' },
  { id: 'colorpicker', name: 'ColorPicker', group: 'Controls', hint: 'Pick a colour', keywords: 'colour swatch', snippet: 'ColorPicker("Colour", selection: .constant(.blue))' },

  // --------------------------------------------------------------- Shapes
  { id: 'rectangle', name: 'Rectangle', group: 'Shapes', hint: 'A filled rectangle', keywords: 'box square', snippet: 'Rectangle()\n    .fill(.blue)\n    .frame(height: 80)' },
  { id: 'roundedrect', name: 'RoundedRectangle', group: 'Shapes', hint: 'A rectangle with rounded corners', keywords: 'card box corner', snippet: 'RoundedRectangle(cornerRadius: 12)\n    .fill(.blue)\n    .frame(height: 80)' },
  ...
  // ----------------------------------------------------------- Navigation
  { id: 'navstack', name: 'NavigationStack', group: 'Navigation', hint: 'A screen you can push from', keywords: 'navigation title bar', snippet: 'NavigationStack {\n    Text("Screen")\n        .navigationTitle("Title")\n}' },
  { id: 'navlink', name: 'NavigationLink', group: 'Navigation', hint: 'Pushes another screen', keywords: 'push detail', snippet: 'NavigationLink("Details") {\n    Text("Details")\n}' },
  { id: 'tabview', name: 'TabView', group: 'Navigation', hint: 'Tabs along the bottom', keywords: 'tabs pages', snippet: 'TabView {\n    Text("First")\n        .tabItem { Label("First", systemImage: "1.circle") }\n    Text("Second")\n        .tabItem { Label("Second", systemImage: "2.circle") }\n}' },
]
  ...
  for (const snippet of VIEW_CATALOG) {
    ...
    const score = name.startsWith(needle)
      ...
      : name.includes(needle)
        ? 1
        : `${snippet.keywords ?? ''} ${snippet.hint}`.toLowerCase().includes(needle)
          ? 2
          : -1
