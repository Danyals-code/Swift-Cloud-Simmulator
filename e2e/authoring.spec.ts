...
test('inspects source provenance without changing the document', async ({ page }) => {
  ...
  await inspector.getByText('Source values and ownership', { exact: true }).click()
  await expect(inspector).toContainText('Literal')
  await expect(inspector).toContainText('Inherited')
...
test('computed content stays read-only and source reveal opens the corresponding code', async ({ page }) => {
  ...
  const inspector = page.getByTestId('authoring-inspector')
  await inspector.getByText('Source values and ownership', { exact: true }).click()
  await expect(inspector).toContainText('Computed')
...
test('design-mode selection never runs a button action', async ({ page }) => {
  await openSource(page)
  await page.getByTestId('render-tree').getByText('Increment', { exact: true }).click()
...
test('designer content edits update Swift, preview and layers; Escape cancels a draft', async ({ page }) => {
  ...
  await expect(page.getByRole('treeitem', { name: 'Designer card, Text' })).toHaveAttribute('aria-selected', 'true')
...
test('restyles a card through supported fields without replacing computed content', async ({ page }) => {
  ...
  await inspector.getByRole('combobox', { name: 'Background', exact: true }).selectOption('blue')
  await expect(inspector.getByRole('combobox', { name: 'background', exact: true })).toHaveValue('blue')
  await inspector.getByRole('combobox', { name: 'Width sizing', exact: true }).selectOption('Fixed')
  await expect(inspector.getByRole('textbox', { name: 'frame · width', exact: true })).toHaveValue('100')
  await page.getByTestId('workspace-develop').click()
  expect(await currentSource(page)).toContain('Text("Count \\(count)").font(.system(size: 24)).background(Color.blue).frame(width: 100)')
})
...
test('an opacity drag is one undo step and Escape cancels the whole drag', async ({ page }) => {
  ...
  await selectBeta(page)
  const inspector = page.getByTestId('authoring-inspector')
  const slider = inspector.getByRole('slider', { name: 'Opacity slider', exact: true })
  ...
  await page.mouse.up()
  await expect(inspector.getByRole('textbox', { name: 'Opacity', exact: true })).toHaveValue('')
  await page.mouse.move(bounds.x + bounds.width * 0.8, bounds.y + bounds.height / 2)
  await page.mouse.down()
  await page.mouse.move(bounds.x + bounds.width * 0.5, bounds.y + bounds.height / 2, { steps: 5 })
  await page.mouse.up()
  await expect(inspector.getByRole('textbox', { name: 'opacity', exact: true })).not.toHaveValue('')
  await page.getByTestId('workspace-develop').click()
  const editor = page.getByTestId('editor').locator('.cm-content')
  await editor.click()
  await page.keyboard.press('ControlOrMeta+z')
