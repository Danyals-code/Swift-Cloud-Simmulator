...
export interface StudioMetadata {
  ...
  readonly components: readonly {
    ...
    readonly properties: readonly { readonly name: string; readonly label: string; readonly description: string }[]
  }[]
  readonly scenarios: readonly {
    readonly name: string
    readonly owner: string
    /** A named developer hook. Scenario input support is added with the scenario phase. */
...
export function readStudioMetadata(value: unknown): MetadataRead {
  ...
  if (!entries(value.labels, v => string(v.owner) && string(v.fingerprint) && string(v.label)) ||
      ...
      !entries(value.components, v => string(v.owner) && entries(v.properties, p => string(p.name) && string(p.label) && string(p.description))) ||
      !entries(value.scenarios, v => string(v.name) && string(v.owner) && string(v.hook))) {
