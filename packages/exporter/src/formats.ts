...
export function buildSwiftPMAppBundle(project: Project): ExportBundle {
  ...
  if (project.assets?.length) for (const [path, bytes] of assetCatalog(project, `${root}/Sources/${name}/Resources/Assets.xcassets`)) put(path, bytes)
...
let package = Package(
    ...
    targets: [
        .executableTarget(
            name: "${name}",
            path: "Sources/${name}"${project.assets?.length ? ', resources: [.process("Resources")]' : ''}
        )
    ]
)
`
...
export function buildPackageBundle(project: Project): ExportBundle {
  ...
  if (project.assets?.length) for (const [path, bytes] of assetCatalog(project, `${name}/Sources/${name}/Resources/Assets.xcassets`)) put(path, bytes)
...
let package = Package(
    ...
    targets: [
        .target(name: "${name}", path: "Sources/${name}"${project.assets?.length ? ', resources: [.process("Resources")]' : ''})
    ]
)
`
}
...
export function buildXcodeGenBundle(project: Project): ExportBundle {
  ...
  if (project.assets?.length) for (const [path, bytes] of assetCatalog(project, `${name}/Sources/Assets.xcassets`)) put(path, bytes)
