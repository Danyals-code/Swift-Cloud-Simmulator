...
export function buildSwiftPMAppBundle(project: Project): ExportBundle {
  ...
  for (const source of sourcesOf(project)) {
    files.set(`${root}/Sources/${name}/${source.path}`, encodeText(source.text))
  }

  files.set(`${root}/Package.swift`, encodeText(appPackageManifest(project)))
  files.set(`${root}/README.md`, encodeText(swiftpmReadme(project)))
  files.set(`${root}/.gitignore`, encodeText(gitignoreContents()))
...
export function buildPackageBundle(project: Project): ExportBundle {
  ...
  for (const source of sourcesOf(project)) {
    files.set(`${name}/Sources/${name}/${source.path}`, encodeText(source.text))
  }

  files.set(`${name}/Package.swift`, encodeText(libraryPackageManifest(project)))
  files.set(`${name}/README.md`, encodeText(packageReadme(project)))
  files.set(`${name}/.gitignore`, encodeText(gitignoreContents()))
...
export function buildXcodeGenBundle(project: Project): ExportBundle {
  ...
  for (const source of sourcesOf(project)) {
    files.set(`${name}/Sources/${source.path}`, encodeText(source.text))
  }

  files.set(`${name}/project.yml`, encodeText(xcodeGenSpec(project)))
  files.set(`${name}/README.md`, encodeText(xcodeGenReadme(project)))
