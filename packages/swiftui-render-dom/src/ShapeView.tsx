...
import { cssColor, cssFill, shapePath, type ShapePayload, type PathPayload } from '@studio/shared'
...
export function ShapeView({ shape, width, height }: { shape: ShapePayload; width: number; height: number }) {
  ...
  return <VectorPathView path={{ d: shapePath(shape.shape, width, height, shape.cornerRadius, shape.cornerStyle, inset), fill: shape.fill, stroke: shape.stroke }} width={width} height={height} />
}

/** Paths and built-in shapes share gradient and stroke painting. */
export function VectorPathView({ path, width, height }: { path: PathPayload; width: number; height: number }) {
