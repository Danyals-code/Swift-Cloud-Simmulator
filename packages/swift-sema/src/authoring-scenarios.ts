...
export function scenarioFiles(ctx: FeatureContext, snapshot: AuthoringSnapshot, scenario?: PreviewScenario): readonly SourceFileNode[] {
  ...
  for (const input of scenario.inputs ?? []) {
    ...
    else if (state && !Array.isArray(input.value)) text = state.type === 'Date' ? `Date(timeIntervalSince1970: ${input.value})` : state.type === 'Color' ? styleExpression('color', input.value as string) : state.options ? '.' + input.value : swiftValue(input.value as DesignValue)
