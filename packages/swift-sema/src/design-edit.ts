...
export function planDesignEdit(request: DesignEditRequest): DesignEditPlan {
  ...
  if (operation.kind === 'property' && node) {
    ...
    const offset = request.target.start
  ...
  const authoring = buildAuthoringModel({ projectId: request.projectId, revision: request.authoringRevision ?? 0, files: nextFiles, parsed: ast.map(f => f.span.file === file.id ? next.sourceFile : f), diagnostics: nextDiagnostics, deploymentTarget: request.deploymentTarget })
