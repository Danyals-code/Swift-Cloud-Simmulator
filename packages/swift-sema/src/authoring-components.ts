...
export function componentRecipes(ctx: FeatureContext, node: AuthoringNode): ComponentRecipe[] {
  ...
  for (const field of fields) {
    ...
    if (scalar && !scalar.optional && input !== undefined && input !== null) {
      ...
      options = type.name === 'Color' && !allDeclarations(ctx).some(d => 'name' in d && d.name === 'Color') ? AUTHORING_COLORS.filter(c => Number.parseFloat(ctx.deploymentTarget ?? '17.0') >= 15 || !['mint', 'teal', 'cyan', 'indigo', 'brown'].includes(c)) : enumCases(ctx, type.name)
